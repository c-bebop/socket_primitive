...
\noindent The important part is the late Middle English origin in the sense 'original, not derivative'. In computer science we could understand the word \textit{primitive} to represent the lowest level representation on the current abstraction level: Original, not derivative.
...
\noindent To demonstrate that a socket primitive can either be a self-contained operation for a programmer who uses a library but not for the implementer of the library, I choose the send primitive for example:
